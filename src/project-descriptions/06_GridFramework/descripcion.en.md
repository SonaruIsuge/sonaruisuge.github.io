...
I develop the framework in my spare time as a technical research project. Instead of implementing gameplay first, I have focused on designing reusable systems that can be extended in future projects. Current features include a generic grid system, optimized asynchronous A* pathfinding, NPC navigation, and a modular building placement system.
...
> **Designed a reusable generic grid framework that decouples gameplay logic from grid management, enabling multiple systems to share the same underlying infrastructure.**
...
To improve search efficiency, the open set is implemented using a binary heap instead of a linear container. Removing the node with the lowest evaluation cost is therefore reduced from O(N) to O(log N), significantly improving performance on larger maps.
...
> **Developed a modular building system on top of the generic grid framework, supporting multi-layer placement, object rotation, and reusable placement logic.**
